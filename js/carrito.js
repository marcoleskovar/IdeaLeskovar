...
const renderCarrito = () =>{
    ...
    for(const elegido of productosDelCarrito){
        ...
        <svg class="carrito-main__sect--list--product--trash" onclick="loaderCarrito(${elegido.id})" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 16 16">
...
const recuperarEliminado = () => {
    const carrito = importarCarrito();
    const recuperado =  JSON.parse(localStorage.getItem('eliminado'))
    carrito.length === 0 && location.reload();
    carrito.push(recuperado);
